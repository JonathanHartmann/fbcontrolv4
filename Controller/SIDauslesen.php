...
include ('configGetSID.php');
$sid_file = (file_get_contents('./httpdocs/websites/8a5.de/sid.txt'));
$heute = date("Y-m-d H:i:s");
$username = $fb_username;
$password = $fb_password;
$loginurl = $fb_loginurl;
print "\n" . $heute . "  Starte SID Script. Bisherige SID: " . $sid_file . "\n";
$sid = get_sid($loginurl, $username, $password, $sid_file);


function get_sid($loginurl, $username, $password, $sid_file)
{
    $heute = date("Y-m-d H:i:s");
    ...
    if (preg_match("/^[0]+$/", $sid) && $challenge) {
        ...
        if ((strlen($sid) > 0) && ! preg_match("/^[0]+$/", $sid)) {
            ...
            file_put_contents('./httpdocs/websites/8a5.de/sid.txt', $sid);
            return $sid;
        }
    } else {
...
$sid__file = (file_get_contents('./httpdocs/websites/8a5.de/sid.txt'));
